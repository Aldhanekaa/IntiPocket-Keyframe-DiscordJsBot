...
module.exports = {
  ...
  async execute(interaction, clientId, extInteractionId) {
    ...
    try {
      ...
      // Get all pockets that the user has access to (owned or is a manager)
      ...
      if (accessiblePockets.length === 0) {
        await interaction.editReply("You don't have access to any pockets.");
        return;
      }
...
async function handleModalSubmit(interaction, clientId, extInteractionId) {
  if (interaction.customId !== undefined) {
    switch (extInteractionId) {
      case `client.${slashCommand.toJSON().name}.schedule_modal`:
        let pocketData;
        try {
